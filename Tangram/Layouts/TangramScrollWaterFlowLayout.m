//
//  TangramScrollWaterFlowLayout.m
//  Tangram
//
...
@interface TangramScrollWaterFlowLayout() <UIScrollViewDelegate>
...
@implementation TangramScrollWaterFlowLayout
...
-(UIScrollView *)innerScrollView
{
    ...
    if (![self.subviews containsObject:_innerScrollView]) {
        [super addSubview:_innerScrollView];
    }
    _innerScrollView.vv_height = self.vv_height;
...
- (void)seutpPagingItemView
{
    ...
    for (NSInteger i = 0; i < self.pagingViews.count; i++) {
        ...
        subView.autoresizingMask    = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        //subView.backgroundColor = [UIColor colorWithString:[self.bgColors stringAtIndex:i]];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (tempPagingIndex == (int)tempPagingIndex) {
        ...
        //发消息
        TangramEvent *didMoveEvent = [[TangramEvent alloc]initWithTopic:@"ScrollFlowLayoutDidMoveToPage" withTangramView:((TangramView *)self.superview) posterIdentifier:self.identifier andPoster:self];
